...
(function(exports) {
  ...
  Socket.prototype._connect = function () {
    ...
    GET(self.url, function (err, xhr) {
      if (err) return self.emit('error', err);

      var wsUrl = xhr.getResponseHeader('X-Harite-Endpoint');
      if (!wsUrl) {
        return self.emit('error', 'X-Harite-Endpoint header not found');
      }
          ...
          if (data.room) {
            ...
            if (keys.length === 3) {
              if (keys[0] === self.applicationId) {
                ...
                self.emit('message', keys[1], keys[2], msg);
              }
            }
          }
        } catch (e) {
